...
class TestInitializers:
    ...
    def test_block_triangularization(self, model):
        ...
        # Because we're using a stoichiometric feed, the actual solution
        # has concentrations of NaOH and Ethyl Acetate equal to zero.
        # Because the Jacobian loses rank at that point, no solver will
        # reach those values.
        assert value(model.fs.unit.outlet.conc_mol_comp[0, "NaOH"]) == pytest.approx(
            0.00015625, rel=2e-5
        )
        assert value(
            model.fs.unit.outlet.conc_mol_comp[0, "EthylAcetate"]
        ) == pytest.approx(0.00015625, rel=2e-5)

        assert value(
            model.fs.unit.outlet.conc_mol_comp[0, "SodiumAcetate"]
        ) == pytest.approx(100.000, rel=2e-5)
        assert value(model.fs.unit.outlet.conc_mol_comp[0, "Ethanol"]) == pytest.approx(
            100.000, rel=2e-5
        )
        assert value(model.fs.unit.outlet.temperature[0]) == pytest.approx(
