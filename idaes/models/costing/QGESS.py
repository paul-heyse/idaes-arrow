...
class QGESSCostingData(FlowsheetCostingBlockData):
    ...
    def build_global_params(self):
        ...
        print("\n" + "=" * 84)
        print(f"{self.local_name} global parameters")
        print("-" * 84)
    ...
    def build_process_costs(
        ...
        feedstock_rate=None,
        production_rate=None,  # this could be power, total REE, water
        CO2_capture_rate=None,  # have CO2 capture be separate from other production
        ...
        transport_per_unit_CO2_cost=None,
    ):
        """
        ...
        Args:
            ...
            production_rate: Var, Param, or Expression for the production rate of power
                ...
                will be calculated and reported.
            CO2_capture_rate: Var, Param, or Expression for the capture rate of CO2.
                If passed, cost of capture will be calculated and reported.
            ...
            transport_per_unit_production_cost: Expression, Var or Param to calculate transport costs of product
                per unit of product.
            transport_per_unit_CO2_cost: Expression, Var or Param to calculate transport costs of CO2
                per unit of CO2.
        ...
        if land_cost is not None:
            ...
            if expr_units == pyunits.dimensionless:
                raise UnitsError("Expression land_cost has no units defined.")
        ...
        if self.config.tech == 10:
            ...
            if self.config.has_variable_OM:
                ...
                if fuel is not None:
                    ...
                    for i in fuel:
                        if i in self.non_fuel_feedstock_waste_resources:
                            del self.non_fuel_feedstock_waste_resources[
                                i
        ...
        # transport cost of production, feedstock and CO2 capture
        ...
        if (
            ...
            try:
                ...
                raise InconsistentUnitsError(
                    "production_rate * transport_per_unit_production_cost",
        ...
        if transport_per_unit_feedstock_cost is not None and feedstock_rate is not None:
            try:
                ...
                raise InconsistentUnitsError(
                    "feedstock_rate * transport_per_unit_feedstock_cost",
        ...
        if transport_per_unit_CO2_cost is not None and CO2_capture_rate is not None:
            try:
                ...
                raise InconsistentUnitsError(
                    "CO2_capture_rate * transport_per_unit_CO2_cost",
                    "transport_CO2_cost",
                    f"Expression transport_CO2_cost failed to build with error: {e} ",
                )
        else:
            self.transport_CO2_cost = Expression(
                expr=0 * self.CEPCI_units / pyunits.year
        ...
        if feedstock_rate is not None:
            ...
            allowed_feedstock_units = (
                True  # check if feedstock units are compatible with LCOF
            )
        ...
        if CO2_capture_rate is not None:
            ...
            # pylint: disable-next=protected-access
            dim_numerator = pyunits.get_units(
                CO2_capture_rate * pyunits.s
            )._pint_unit.dimensionality

            allowed_CO2_capture_units = (
                True  # check if CO2 capture units are compatible with LCOC
            )
            ...
            if (
                ...
                allowed_CO2_capture_units = False

            if not allowed_CO2_capture_units:
                raise UnitsError(
                    f"The argument CO2_capture_rate was passed with the units {pyunits.get_units(CO2_capture_rate)} "
                    f"with dimensions {dim}. Please ensure that the CO2 capture rate has units of valid_units/time."
                )

            self.additional_cost_of_CO2_capture = Var(
                initialize=0,
                doc="Additional cost to be added to the LCOC calculations",
                units=self.base_currency
                / pyunits.get_units(CO2_capture_rate * pyunits.year),
            )
            self.additional_cost_of_CO2_capture.fix()

            self.cost_of_CO2_capture = Expression(
                expr=(
                    pyunits.convert(
                        (
                            self.annualized_cost / pyunits.year
                        ...
                        * pyunits.year
                        / (CO2_capture_rate * pyunits.year * self.capacity_factor),
                        to_units=self.base_currency
                        / pyunits.get_units(CO2_capture_rate * pyunits.year),
                    )
                    + self.additional_cost_of_CO2_capture
                )
            )
