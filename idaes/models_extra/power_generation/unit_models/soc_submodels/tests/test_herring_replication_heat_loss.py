...
def model():
    ...
    m = model_func()
    m.fs.cell.total_heat_loss.fix(0)
    return m
...
def model_func_log_exchange_current_modifier():
    ...
    m.fs.cell.fuel_triple_phase_boundary.log_exchange_current_modifier.fix(0)
    m.fs.cell.oxygen_triple_phase_boundary.log_exchange_current_modifier.fix(0)
    m.fs.cell.total_heat_loss.fix(0)
...
def model_lecm():
    # Want to be able to call model_func when calling this test directly
    # so that the reference dataset can be recreated if necessary
    return model_func_log_exchange_current_modifier()


@pytest.mark.skipif(not helmholtz_available(), reason="General Helmholtz not available")
@pytest.mark.component
def test_initialization_cell_log_exchange_current_modifier(model_lecm):
    m = model_lecm
    cell = m.fs.cell
...
@pytest.mark.skipif(not helmholtz_available(), reason="General Helmholtz not available")
@pytest.mark.integration
def test_model_replication_log_exchange_current_modifier(model_lecm):
    out = kazempoor_braun_replication(model_lecm)
    cached_results = []
    for i in range(len(out)):
        cached_results.append(
            pd.read_csv(
