...
- `deltalake` Python + DataFusion-Delta integration: out of scope here. The DataFusion deep-dives intentionally stop at the DataFrame/SessionContext/table-provider boundary; Delta integration is a downstream concern. For the Rust side of Delta see the `deltalake-rust-ref` skill.
- DataFusion Rust UDFs and FFI: out of scope here. The Python UDF/UDAF/UDWF/UDTF surface is covered by this skill; Rust-side custom kernels and the FFI boundary contract belong to `datafusion-pyarrow-rust-ref`.
