...
The check earns its keep. It caught a defect in the Python extractor, which
counted a multi-line `@deprecated(...)` decorator twice — once as a call at the
decorator line and once as a decorator at the class line — inflating the
deprecation count from 49 to 53. The tree-sitter rule matched each site once.
...
| `enums.csv` | 55 | Enum / IntEnum / StrEnum subclasses with members and values |
...
- **45** classes in the tree are named `*Scaler`, but only **24** process block
